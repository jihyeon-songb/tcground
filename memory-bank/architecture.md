...
- KREAM 자동 source(`kream`)는 판매중 호가만 수집한다. JSON API 기반 product option 수집은 `aggregation_method='kream_asking_median'` snapshot으로 저장하고, JSON API가 500을 반환하는 경우에는 렌더된 검색 페이지의 상품 카드에서 현재 표시가를 읽어 `aggregation_method='kream_search_page_asking'` snapshot으로 저장한다. 검색 페이지 수집은 상품명+세트명+레어도 매칭 confidence가 충분하고 동률 후보가 없을 때만 저장한다. 기존 `manual_kream` CSV 행은 사람이 검증한 과거 체결(sold) evidence로 보존하며, 자동 KREAM asking snapshot과 섞지 않는다.
