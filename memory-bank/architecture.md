...
> 마지막 갱신: 2026-07-07 (상세 판매 호가·외부 링크 정책)
...
- 카드 이름 검색은 독립 라우트가 아니라 `/categories/[categoryId]?q=...` 쿼리로 동작한다. `q`가 있으면 `lib/tcg-catalog.ts`의 `getPokemonCategoryPageData`가 Supabase `cards.name`에 `ilike '%q%'` 필터를 적용하고, 페이지는 세트 그리드를 숨긴 채 결과 인디케이터와 매칭 카드 그리드만 노출한다. 카테고리 정렬은 기본 `추천순`(`get_cards_by_price_sample_count()`), `가격 높은순`(`get_cards_by_average_asking_price()`), 이름순을 지원하며 가격 데이터가 없는 카드는 slug 순 fallback으로 뒤에 채운다. 헤더 검색창의 `initialQuery`/`showClearButton`은 그대로 유지한다.
...
- 가격 수집 모듈: `lib/pricing/`. source-agnostic 어댑터 계약(`price-source.types.ts`), 관측치→snapshot 집계(`aggregate.ts`), 수동 CSV import(`csv-import.ts`), 환율 fetch/display 환산(`fx.ts`), eBay 어댑터(`ebay/`), 국내 marketplace 어댑터(`bunjang/`, `joongna/`, `kream/`)를 둔다. 모든 일일 수집은 `lib/pricing/collect-prices.ts`를 호출하고, RLS deny-all인 `price_observations`/`card_price_snapshots`/`price_collection_runs` 쓰기는 service-role 클라이언트(`lib/supabase/admin.ts`)로만 한다. 수집은 IP 제약에 따라 두 경로로 갈린다. (1) **eBay 전수(GitHub Actions)**: eBay Browse는 거주 IP가 필요 없는 OAuth API라 `.github/workflows/collect-ebay-daily.yml`이 매일 03:00 KST(18:00 UTC) 클라우드에서 `pnpm collect:ebay:daily`(=`scripts/collect-prices.ts --browse`)를 실행해 전체 카탈로그(약 4,677장)를 수집한다. DB·eBay 자격은 repo secrets로 주입하고 워크플로가 `.env.local`을 임시 생성한다. Browse 검색 한 번이 fixed-price·auction listing을 함께 반환하므로 `collectBrowseAuctionSnapshots`가 item별 `buyingOptions`로 분리해 `ebay_browse`(호가)와 `ebay_auction`(경매) snapshot을 한 콜로 만든다 → 카드당 1콜, 전수 4,677콜/일이 Browse API 5,000콜/일 한도 안에 든다. (2) **로컬 launchd(KREAM)**: 데이터센터 IP에서 막히는 KREAM은 거주 IP가 필요해 `scripts/launchd/com.tcground.daily-price-collection.plist`가 매일 03:00 `pnpm collect:daily`(=`collect:kream:daily`)를 실행한다. KREAM은 `scripts/collect-kream-search-page.ts`로 검색 페이지 렌더 결과의 현재 표시가를 수집하며, 단일 검색어 약 50개 한계를 줄이려 catalog 세트명/세트명+레어도 검색어를 순회하는 segmented search를 쓰고 product id로 중복 제거한다. eBay/KREAM처럼 외부 fetch가 불안정한 source는 요청 timeout과 연속 실패 abort를 둬 한 source가 장시간 잡을 붙잡지 않게 한다. (이전의 Vercel Cron `app/api/cron/collect-prices/route.ts`와 날짜별 50장 rotating window는 GitHub Actions 전수 이전으로 제거됨. 가격 캐시는 `PRICES_CACHE_TAG`의 시간기반 revalidate(1시간)로 갱신된다.) 로컬 검증/수동 sold/asking import와 FX import는 `scripts/collect-prices.ts`. 전체 한국판 카탈로그의 증거 대기 목록 동기화는 `scripts/sync-price-worklist.ts`가 담당하며, CSV `sample_id`는 `PKMKR-<card_num>`으로 통일해 `card_printings.external_ids.card_num`으로 해소한다. 기존 `KR-*` priority 번호는 `raw_payload_json.worklist_id` alias로만 보존한다.
- eBay 가격 source 현실 제약: 실거래가(sold)는 Marketplace Insights API(`buy.marketplace.insights`)로만 얻고 이는 Limited Release라 개인 개발자 승인이 어렵다. `marketplace-insights-adapter.ts`는 매핑/scaffold만 두고 `EBAY_MARKETPLACE_INSIGHTS_ENABLED=true`가 아니면 `EbayAccessNotGrantedError`를 던진다. 일별 시계열은 개인도 쓸 수 있는 Browse API(`browse-adapter.ts`, 판매중 호가) 일일 수집으로 누적하고, 수동 CSV sold 실거래가는 차트 참조점으로 오버레이한다. 상품 상세 진입 시에는 `lib/pricing/ebay/current-asking.ts`가 선택된 printing의 당일 `ebay_browse` snapshot을 on-demand로 갱신하되, 이미 당일 snapshot이 있으면 API를 재호출하지 않는다. 이 on-demand 호출은 3.5초 타임아웃과 credential/permission fallback을 둬 상세 렌더를 막지 않는다. eBay 검색어는 `card_printings.external_ids.name_en`/`name_ja`를 우선하고 한국어 카드명으로 fallback한다. 카드 상세 차트(`app/cards/[cardId]/page.tsx`)는 `card_price_snapshots`를 읽어 그리며 시장/통화는 섞지 않는다.
- KREAM 자동 source(`kream`)는 판매중 호가만 수집한다. JSON API 기반 product option 수집은 `aggregation_method='kream_asking_median'` snapshot으로 저장하고, JSON API가 500을 반환하는 경우에는 렌더된 검색 페이지의 상품 카드에서 현재 표시가를 읽어 `aggregation_method='kream_search_page_asking'` snapshot으로 저장한다. 검색 페이지 수집은 기본 검색어와 catalog 세트명/레어도 segment 검색어를 순회하고 product id로 중복 제거한 뒤, 상품명+세트명+레어도 매칭 confidence가 충분하고 동률 후보가 없을 때만 저장한다. 기존 `manual_kream` CSV 행은 사람이 검증한 과거 체결(sold) evidence로 보존하며, 자동 KREAM asking snapshot과 섞지 않는다.
- 수동 CSV sold 집계는 source별로 분리해 `card_price_snapshots.source_name`에 `ebay_sold`, `pricecharting_ebay_sold`, `manual_kream`, `manual_bunjang` 같은 원천을 보존한다. `manual_bunjang`/`manual_joongna`처럼 같은 source가 sold/asking 양쪽에 쓰일 수 있고, `kream`도 과거 legacy sold snapshot과 새 asking snapshot이 공존할 수 있으므로 상세 차트는 `source_name`만 보지 않고 `aggregation_method`(`median_filtered` vs `manual_asking_median`/`*_asking_median`)를 우선해 sold/asking을 구분한다. 대표 가격 요약 source label은 asking snapshot만 `eBay`, `KREAM`, `번개장터`, `중고나라` 등 사용자에게 읽히는 이름으로 노출한다. `PriceCharting eBay sold` 같은 sold-source label은 차트 참고 데이터에만 표시한다.
- 카드 상세 외부 링크는 검증된 eBay listing → 실제 asking source URL → eBay 검색 결과 순서로 선택한다. listings가 없는 레거시 `ebay_browse.source_url`은 직접 링크로 사용하지 않는다.
...
- 중고나라 자동 source: `lib/pricing/joongna/joongna-adapter.ts`는 공개 search page hydration 데이터에서 상품 `seq`, 제목, 가격, 정렬일 같은 최소 필드만 추출한다. `joongna`는 실거래가가 아닌 KRW `asking` source이며 `aggregation_method='joongna_asking_median'`으로 저장한다. 검색은 카드명 중심으로 수행하고, snapshot 생성은 카드명/세트/번호 confidence와 중고나라 전용 제외어(박스/팩/미개봉/일괄/해외판/포켓 앱 카드 등)를 통과한 listing만 허용한다.
- 데이터 최소화: eBay/CSV/국내 marketplace 관측치는 가격·일자·상태·등급·item id/url·축소 `raw_payload`만 저장하고 seller/buyer 식별 정보와 원문 전체 payload는 저장하지 않는다.
- 가격 수집/env(server-only): `SUPABASE_SERVICE_ROLE_KEY`, `KOREA_EXIM_FX_API_KEY`, `EBAY_ENV`, `EBAY_CLIENT_ID`, `EBAY_CLIENT_SECRET`, `EBAY_MARKETPLACE_INSIGHTS_ENABLED`, `EBAY_DELETION_VERIFICATION_TOKEN`, `EBAY_DELETION_ENDPOINT`, `KREAM_COLLECTION_ENABLED`, `BUNJANG_COLLECTION_ENABLED`, `JOONGNA_COLLECTION_ENABLED`. eBay 전수 GitHub Actions 워크플로는 `NEXT_PUBLIC_SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY`/`EBAY_CLIENT_ID`/`EBAY_CLIENT_SECRET`를 repo secrets로 받아 실행 시 `.env.local`을 임시 생성한다(`EBAY_ENV=production`은 워크플로에 하드코딩). 로컬 launchd `pnpm collect:daily`는 KREAM asking만 수집하므로 `KREAM_COLLECTION_ENABLED=true`를 명령 환경에서 주입하되, 실제 운영 전 compliant access와 한국/거주용 IP를 확인한다. `CRON_SECRET`은 Vercel Cron 제거로 더 이상 쓰지 않는다. `app/api/ebay/account-deletion/route.ts`는 eBay Marketplace account deletion/closure notification 검증용 HTTPS endpoint이며, eBay portal에 등록한 endpoint URL과 `EBAY_DELETION_ENDPOINT`가 정확히 같아야 한다. placeholder는 `.env.example` 참고.
...
- 2026-05-22: 검증된 한국판 포켓몬 대표 카드 10장을 Supabase 카탈로그에 seed하고, `/categories/pokemon`과 `/cards/[cardId]`를 `lib/tcg-catalog.ts` 기반 DB 조회로 전환.
