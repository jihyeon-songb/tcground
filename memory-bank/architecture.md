...
- 가격 수집 env(server-only): `SUPABASE_SERVICE_ROLE_KEY`, `KOREA_EXIM_FX_API_KEY`, `EBAY_ENV`, `EBAY_CLIENT_ID`, `EBAY_CLIENT_SECRET`, `EBAY_MARKETPLACE_INSIGHTS_ENABLED`, `KREAM_COLLECTION_ENABLED`, `BUNJANG_COLLECTION_ENABLED`, `JOONGNA_COLLECTION_ENABLED`, `CRON_SECRET`. placeholder는 `.env.example` 참고.
