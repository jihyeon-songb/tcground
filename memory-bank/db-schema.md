...
- 카테고리: `card_categories`와 `card_category_links`로 선택 범위에 해당하는 카드를 찾고 최신 가격 스냅샷을 조인한다. 기본 `추천순`은 `get_cards_by_price_sample_count()` ranking을 먼저 적용하고, 가격 데이터가 없는 카드는 slug 순 fallback으로 채운다.
