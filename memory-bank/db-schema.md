...
- FX migration 적용 전에는 앱과 적재 스크립트가 legacy snapshot 컬럼으로 fallback한다.
...
제약/인덱스: `index(price_observations.card_printing_id, sold_at desc)`, `index(price_observations.source_name, market, sold_at desc)`, `index(price_observations.variant, condition_label, grade_company, grade_value)`, `unique(source_name, source_item_id/source_url, sold_at, sold_price, variant, grade_company, grade_value)` where source identifier exists. 단, 원격 DB의 기존 `source_name + source_item_id` unique index는 FX/display migration 적용 전까지 남아 있으므로 KREAM처럼 product id 안에 여러 grade 체결 행이 있는 source는 일부 관측치가 보류될 수 있다.
...
- 2026-06-03: FX/display 가격 확장 설계 추가. `exchange_rates`와 `card_price_snapshots`의 source/display 가격·환율 기준일 컬럼, `price_observations` source id unique index 교체를 `supabase/migrations/202606030001_add_fx_price_display.sql`로 준비했다. 원격 적용 전까지 앱과 적재 스크립트는 legacy snapshot 컬럼으로 fallback한다.
