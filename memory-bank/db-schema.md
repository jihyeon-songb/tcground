...
실제 Supabase 적용은 Supabase MCP 또는 CLI migration으로 진행한다. 현재 MVP 스키마는 `create_tcg_mvp_schema`, `optimize_favorite_cards_rls_policies`, `extend_price_collection_models`, `lock_internal_price_collection_tables`, `add_fx_price_display`, 추천순 ranking migration으로 적용되어 있다. 로컬 reset 기준 public schema baseline은 `supabase/migrations/202605200001_create_local_baseline_schema.sql`이며, FX/display 가격 확장 SQL의 로컬 기준 파일은 `supabase/migrations/202606030001_add_fx_price_display.sql`이다.
...
| `source_currency`    | `text`          | nullable                            | 원천 snapshot 통화               |
| `source_avg_price`   | `numeric(14,2)` | nullable                            | 원천 통화 평균가                 |
| `source_min_price`   | `numeric(14,2)` | nullable                            | 원천 통화 최저가                 |
| `source_max_price`   | `numeric(14,2)` | nullable                            | 원천 통화 최고가                 |
| `display_currency`   | `text`          | nullable                            | UI 표시 통화. MVP 기본 `KRW`     |
| `display_avg_price`  | `numeric(14,2)` | nullable                            | 표시 통화 평균가                 |
| `display_min_price`  | `numeric(14,2)` | nullable                            | 표시 통화 최저가                 |
| `display_max_price`  | `numeric(14,2)` | nullable                            | 표시 통화 최고가                 |
| `fx_rate`            | `numeric(18,8)` | nullable                            | 환산에 사용한 rate               |
| `fx_rate_date`       | `date`          | nullable                            | 실제 사용한 환율 기준일          |
| `fx_provider`        | `text`          | nullable                            | 환율 provider                    |
| `created_at`         | `timestamptz`   | not null, default `now()`           | 생성 시각                        |
...
- `currency`와 `avg_price` 계열은 source snapshot 기준을 유지한다.
- `display_currency`와 `display_*_price`는 UI 비교/표시 기준이다. 한국판 MVP의 기본 표시 통화는 `KRW`다.
- FX migration 미적용 환경에서는 앱과 적재 스크립트가 legacy snapshot 컬럼으로 fallback한다.

### `exchange_rates`

외화 원천 가격을 KRW 등 표시 통화로 환산하기 위한 일별 rate 테이블이다.

| 컬럼             | 타입            | 제약                            | 설명                                  |
| ---------------- | --------------- | ------------------------------- | ------------------------------------- |
| `id`             | `uuid`          | PK, default `gen_random_uuid()` | 내부 식별자                           |
| `base_currency`  | `text`          | not null                        | 원천 통화. 예: `USD`, `GBP`, `JPY`    |
| `quote_currency` | `text`          | not null, default `KRW`         | 표시 통화                             |
| `rate`           | `numeric(18,8)` | not null, check `rate > 0`      | 1 base currency당 quote currency 금액 |
| `rate_date`      | `date`          | not null                        | 환율 기준일                           |
| `provider`       | `text`          | not null                        | 예: `korea_exim`                      |
| `fetched_at`     | `timestamptz`   | not null, default `now()`       | 수집 시각                             |
| `raw_payload`    | `jsonb`         | not null, default `{}`          | provider 응답 최소 보존               |
| `created_at`     | `timestamptz`   | not null, default `now()`       | 생성 시각                             |

제약/인덱스: `unique(base_currency, quote_currency, rate_date, provider)`, `index(exchange_rates.base_currency, quote_currency, rate_date desc)`.

RLS: 공개 `select`만 허용하고 쓰기는 service role 수집 경로에서만 수행한다.
...
제약/인덱스: `index(price_observations.card_printing_id, sold_at desc)`, `index(price_observations.source_name, market, sold_at desc)`, `index(price_observations.variant, condition_label, grade_company, grade_value)`, `unique(source_name, source_item_id/source_url, sold_at, sold_price, variant, grade_company, grade_value)` where source identifier exists.
...
### `card_price_sample_count_rank` (materialized view)

카테고리/홈 기본 `추천순`의 precomputed ranking이다. `card_price_snapshots`를 매 요청마다 집계하지 않고, raw 가격 스냅샷의 `sample_count` 합계를 카드별로 보관한다.

| 컬럼           | 타입     | 설명                                   |
| -------------- | -------- | -------------------------------------- |
| `card_id`      | `uuid`   | 추천순 대상 카드                       |
| `sample_count` | `bigint` | raw 가격 스냅샷의 누적 가격 표본 수 합 |

인덱스: `unique(card_id)`, `index(sample_count desc, card_id asc)`.

### `get_cards_by_price_sample_count()` / `refresh_card_price_sample_count_rank()` (RPC)

`get_cards_by_price_sample_count()`는 `card_price_sample_count_rank`를 `sample_count desc, card_id asc`로 읽어 기본 추천순 카드 ID를 반환한다. `anon`, `authenticated`에 `execute`를 부여한다.

`refresh_card_price_sample_count_rank()`는 daily 가격 수집 후 service role이 호출해 materialized view를 `refresh materialized view concurrently`로 갱신한다. 공개 role에는 실행 권한을 주지 않는다.

### `card_average_asking_price_rank` (materialized view)

카테고리 명시 정렬 `가격 높은순`의 precomputed ranking이다. `card_price_snapshots`의 raw KRW asking snapshot 중 카드별 현재 snapshot 1개를 보관하며, 정렬 값은 판매중 호가 평균(`display_avg_price`)이다. 동일 최신일이면 더 높은 표시 가격을 사용한다.

| 컬럼                   | 타입      | 설명                      |
| ---------------------- | --------- | ------------------------- |
| `card_id`              | `uuid`    | 가격순 대상 카드          |
| `average_asking_price` | `numeric` | raw KRW 평균 판매 호가    |

인덱스: `unique(card_id)`, `index(average_asking_price desc, card_id asc)`.

### `get_cards_by_average_asking_price()` / `refresh_card_average_asking_price_rank()` (RPC)

`get_cards_by_average_asking_price()`는 `card_average_asking_price_rank`를 `average_asking_price desc, card_id asc`로 읽어 가격 높은순 카드 ID를 반환한다. `anon`, `authenticated`에 `execute`를 부여한다.

`refresh_card_average_asking_price_rank()`는 daily 가격 수집 후 service role이 호출해 materialized view를 `refresh materialized view concurrently`로 갱신한다. 공개 role에는 실행 권한을 주지 않는다.
...
- 카테고리: `card_categories`와 `card_category_links`로 선택 범위에 해당하는 카드를 찾고 최신 가격 스냅샷을 조인한다. 기본 `추천순`은 `get_cards_by_price_sample_count()` ranking을 먼저 적용하고, 명시 `가격 높은순`은 `get_cards_by_average_asking_price()` ranking을 적용한다. 가격 데이터가 없는 카드는 slug 순 fallback으로 채운다.
...
- 2026-06-15: KREAM 자동 source(`source_name='kream'`)의 가격 성격을 체결(sold)에서 판매중 호가(asking)로 전환했다. 새 자동 수집 결과는 `price_observations`에 넣지 않고 `card_price_snapshots.aggregation_method='kream_asking_median'`으로 저장한다. 기존 `manual_kream` CSV sold evidence와 과거 `median_filtered` KREAM snapshot은 체결 근거로 보존하므로 **스키마 변경 없음**이며, 상세 차트는 `aggregation_method`를 우선해 sold/asking을 구분한다.
- 2026-06-13: 원격 MCP migration 기록으로만 남아 있던 public schema를 로컬 Supabase reset용 baseline migration(`202605200001_create_local_baseline_schema.sql`)으로 복원했다. 기존 schema 의미 변경은 없으며, 빈 로컬 DB에서 후속 FX/display와 category counts migration을 검증할 수 있게 하는 재현성 보강이다.
- 2026-06-03: FX/display 가격 확장 설계 추가. `exchange_rates`와 `card_price_snapshots`의 source/display 가격·환율 기준일 컬럼, `price_observations` source id unique index 교체를 `supabase/migrations/202606030001_add_fx_price_display.sql`로 준비했다. 미적용 환경에서는 앱과 적재 스크립트가 legacy snapshot 컬럼으로 fallback한다.
- 2026-06-05: Supabase MCP migration `add_fx_price_display`로 원격 DB에 FX/display 가격 확장을 적용했다. 기존 snapshot 112개는 source/display 컬럼으로 backfill됐고, `price_observations` unique index는 source id + 거래 bucket 기준으로 교체됐다.
- 2026-06-04: 수동 CSV sold 집계를 source별로 분리해 `card_price_snapshots.source_name`에 `ebay_sold`, `pricecharting_ebay_sold`, `manual_kream`, `manual_bunjang` 같은 원천을 보존한다. `manual_bunjang`처럼 같은 source가 sold/asking 양쪽에 쓰일 수 있으므로 상세 차트는 `aggregation_method`(`median_filtered` vs `manual_asking_median`/`*_asking_median`)를 우선해 가격 성격을 판정한다.
